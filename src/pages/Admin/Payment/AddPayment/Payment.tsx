...
import {
  ...
  findPaymentById,
  updatePayment,
} from "../../../../_service/payment_service.ts";
...
export default function Payment() {
  ...
  let paymentId = Number(params.uid);
  let msgSuccess: any;
  ...
  const onChangeSelect = (e) => {
    ...
    findAllInvoiceByCustomer(paginationModel, e.target.value).then((data) =>
      setInvoiceByCustomer(data)
    );
  };
  ...
  useEffect(() => {
    ...
    if (paymentId) {
      ...
      // // Load Invoice depending on customer
      findAllInvoiceByCustomer(paginationModel, payment?.idCustomer).then(
        (data) => setInvoiceByCustomer(data)
      );
    }
  }, [paymentId]);
  const fakeRows = [
    {
      ...
      amount: 10000.0, //float. ---> SUM of total price of all travel_item linked to invoice
      status: "", //string
      balance: 0.0, //float
      credit_apply: 0.0, //float
    },
  ];
  ...
  return (
    <PaymentForms
      ...
      payment={payment}
      columns={columns}
      onChangeSelectPay={onChangeSelectPay}
      checkboxSelection={false}
      rows={fakeRows}
      onChangeSelect={onChangeSelect}
      setRowSelectionModel={setRowSelectionModel}
      listOptpayMode={listOptpayMode}
    />
  );
}
